...
export function OutcomesLab({ stats, current, pointInTime = false }: { stats: OutcomeStat[]; current?: Regime; pointInTime?: boolean }) {
  ...
  return <>
    ...
    <section className="signal-strip outcome-signals"><div><span>Selected regime</span><strong>{shortRegime(activeRegime)}</strong><small>{pointInTime ? "ALFRED point-in-time" : "US macro / 2m lag proxy"}</small></div><div><span>Qualified samples</span><strong>{sampleCount}</strong><small>Quarterly-spaced starts</small></div><div><span>Positive median sleeves</span><strong>{positive}/{rows.length}</strong><small>{horizon}-month outcomes</small></div><div><span>Highest median</span><strong>{best?.symbol ?? "--"}</strong><small>{best ? percent(best.median) : "No observations"}</small></div></section>
